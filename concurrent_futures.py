...
def fetch_and_download(pokemon_id: int) -> None:
    ...
    Executada por cada worker do ThreadPoolExecutor individualmente.
    """
...
def run_once(quantity: int, num_workers: int) -> float:
    """
    Executa uma rodada completa de download com ThreadPoolExecutor.
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        futures = {executor.submit(fetch_and_download, pid): pid for pid in ids}
        for future in as_completed(futures):
            future.result()  # propaga exceções não tratadas, se houver

    return time.time() - start_time


def benchmark_futures(quantity: int, num_workers: int, runs: int = RUNS) -> None:
    """
    Executa `runs` vezes o download paralelo com `num_workers` workers
    via concurrent.futures para `quantity` Pokémons.

    Parâmetros
    ----------
    quantity    : Número de Pokémons por rodada (100, 500 ou 1000).
    num_workers : Número de workers simultâneos (2, 4 ou 8).
    runs        : Número de rodadas para a média (padrão: 10).
    """
    print(f"\n{'='*55}")
    print(f"  Benchmark concurrent.futures")
    ...
    print(f"\n  --- Resultados ({quantity} Pokémons, {num_workers} workers, {runs} rodadas) ---")
...
if __name__ == "__main__":
    for quantity in [50, 250, 500]:
        for num_workers in [2, 4, 8]:
            benchmark_futures(quantity=quantity, num_workers=num_workers, runs=RUNS)
